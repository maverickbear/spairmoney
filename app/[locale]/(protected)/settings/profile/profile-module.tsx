...
export function ProfileModule() {
  ...
  useEffect(() => {
    ...
    async function loadUserLocation() {
      try {
        const response = await fetch(apiUrl("/api/v2/onboarding/location"));
        if (response.ok) {
  ...
  async function loadProfile(force = false) {
    ...
    try {
      ...
      const fetchPromise = (async () => {
        const response = await fetch(apiUrl("/api/v2/profile"));
        if (!response.ok) {
  ...
  async function onSubmit(data: ProfileFormData) {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || t("failedToUpdateProfile"));
      }
  ...
  async function handleFileUpload(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file.type.startsWith("image/")) {
      toast({ title: tCommon("error"), description: t("pleaseUploadImage"), variant: "destructive" });
      return;
    }

    // Validate file size (max 5MB)
    if (file.size > 5 * 1024 * 1024) {
      toast({ title: tCommon("error"), description: t("imageMaxSize5MB"), variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      const res = await fetch(apiUrl("/api/v2/profile/avatar"), {
        method: "POST",
        body: formData,
      });
      ...
      try {
        ...
        toast({ 
          title: t("avatarUploadedTitle"), 
          description: t("avatarUploadedButFailedToSave"), 
          variant: "destructive" 
        });
        return;
      }
      
      toast({ title: t("avatarUploadSuccessTitle"), description: t("avatarUploadSuccessDescription"), variant: "success" });
    } catch (error) {
      console.error("Error uploading avatar:", error);
      toast({
        title: tCommon("error"),
        description: error instanceof Error ? error.message : t("failedToUploadAvatar"),
        variant: "destructive"
      });
    } finally {
      setUploading(false);
          ...
          <div className="flex flex-col gap-4 sm:gap-6">
            ...
            <div className="flex flex-row items-start gap-4">
              <div className="relative flex-shrink-0">
                ...
                >
                  {getInitials(form.watch("name") || profile?.name || t("userFallback"))}
                </div>
              </div>
              ...
              <div className="flex flex-col justify-center space-y-1 flex-1">
                ...
                >
                  ...
                  {uploading ? tCommon("uploading") : t("uploadAvatar")}
                </Button>
