...
export default function BudgetsPage() {
  ...
  async function loadData() {
    try {
      ...
      const [budgetsResponse, categoriesResponse] = await Promise.all([
        fetch(apiUrl(`/api/v2/budgets?period=${periodParam}`)),
        fetch(apiUrl("/api/v2/categories?all=true")),
      ]);
  ...
  async function handleCreateBudget(categoryId: string) {
    ...
    try {
      const response = await fetch(apiUrl("/api/v2/budgets"), {
        method: "POST",
  ...
  return (
    <div>
      <PageHeader title={t("items.budgets")} />
        ...
        {loading && !hasLoaded ? (
          <Card>
            <CardContent className="py-12 text-center">
              <div className="text-muted-foreground">{tPlan("loadingCategories")}</div>
            </CardContent>
          </Card>
        ) : categories.length === 0 ? (
          <div className="w-full h-full min-h-[400px]">
            <EmptyState
              icon={Wallet}
              title={tPlan("noCategoriesFound")}
              description={tPlan("noCategoriesFoundDescription")}
            />
          </div>
      ...
      <Sheet open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <SheetContent side="right" className="sm:max-w-[600px] w-full p-0 flex flex-col gap-0 overflow-hidden bg-background border-l">
          <SheetHeader className="p-6 pb-4 border-b shrink-0">
            <SheetTitle className="text-xl">{tPlan("manageBudgetCategories")}</SheetTitle>
            <SheetDescription>
              {tPlan("manageBudgetCategoriesDescription")}
            </SheetDescription>
          </SheetHeader>
                    ...
                    return (
                      ...
                      >
                        ...
                        <Button
                          ...
                          aria-label={hasBudget ? tPlan("deleteBudget") : tPlan("createBudget")}
                        >
                          {isProcessing ? (
