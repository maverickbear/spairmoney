...
export function GoalsTab() {
  ...
  function handleDelete(id: string) {
    openDialog(
      {
        title: tPlanning("deleteGoal"),
        description: tPlanning("deleteGoalConfirm"),
        variant: "destructive",
  ...
  return (
    <div className="space-y-6">
      ...
      {loading && !hasLoaded ? (
        ...
        <EmptyState
          icon={PiggyBank}
          title={filterBy === "all" ? tPlanning("noGoalsYet") : tPlanning("noGoalsFilterFound", { filter: filterBy })}
          description={
            filterBy === "all"
              ? tPlanning("noGoalsYetDescription")
              : tPlanning("noGoalsFilterDescription", { type: filterBy === "active" ? tPlanning("completed") : tPlanning("active") })
          }
        />
        </div>
      ) : (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold">
              {filterBy === "all" ? tPlanning("allGoals") : filterBy === "active" ? tPlanning("activeGoalsLabel") : tPlanning("completedGoalsLabel")} ({sortedGoals.length})
            </h3>
          </div>
