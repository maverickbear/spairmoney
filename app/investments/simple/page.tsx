...
export default function SimpleInvestmentsPage() {
  ...
  async function loadData() {
    try {
      ...
      const [accountsRes, entriesRes] = await Promise.all([
        fetch("/api/accounts"),
        fetch("/api/investments/simple/entries"),
      ]);
      ...
      const valuesPromises = investmentAccounts.map((acc: InvestmentAccount) =>
        fetch(`/api/investments/simple/accounts/${acc.id}/value`)
          .then((r) => r.json())
          .catch(() => null)
      );
  ...
  async function handleSubmitEntry(data: InvestmentEntryFormData) {
    try {
      const response = await fetch("/api/investments/simple/entries", {
        method: "POST",
  ...
  async function handleSubmitValue(data: AccountValueFormData) {
    try {
      const response = await fetch(`/api/investments/simple/accounts/${data.accountId}/value`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        ...
        <Card>
          <CardContent className="py-8 text-center text-muted-foreground">
            ...
            <p className="text-sm mt-2">
              Create an account with type "investment" in the Accounts page first.
            </p>
          </CardContent>
        </Card>
      ) : (
          ...
          <form onSubmit={entryForm.handleSubmit(handleSubmitEntry)} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <MoneyInput {...entryForm.register("amount", { valueAsNumber: true })} />
        ...
        <DialogContent>
          ...
          <form onSubmit={valueForm.handleSubmit(handleSubmitValue)} className="space-y-4">
            <div>
              ...
              <MoneyInput {...valueForm.register("totalValue", { valueAsNumber: true })} />
