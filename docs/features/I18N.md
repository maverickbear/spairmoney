...
The **LocaleSwitcher** component (`components/common/locale-switcher.tsx`) is used in the landing header and footer, and the **user avatar menu** has a Language section (EN | PT | ES). The language selector is visible in both development and production so users can switch locale at any time.
