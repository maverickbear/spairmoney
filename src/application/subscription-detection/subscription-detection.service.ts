...
export class SubscriptionDetectionService {
  ...
  async detectSubscriptionsFromTransactions(userId: string): Promise<DetectedSubscription[]> {
    try {
      ...
      logger.info(`[SubscriptionDetectionService] Detecting subscriptions for user: ${userId}`);
      ...
      if (error) {
        logger.error("[SubscriptionDetectionService] Error fetching transactions:", error);
        throw new AppError("Failed to fetch transactions", 500);
      }
      
      if (!transactions || transactions.length === 0) {
        logger.debug("[SubscriptionDetectionService] No transactions found");
        return [];
      }
      ...
      for (const [key, group] of merchantGroups.entries()) {
        ...
        const dateRegularity = this.calculateDateRegularity(dates);
        ...
        const knownService = this.findKnownSubscriptionService(group.merchantName);
        ...
        // Calculate frequency
        const frequency = knownService?.typicalFrequency || this.calculateFrequency(dates);
        const billingDay = this.calculateBillingDay(frequency, dates);
      ...
      logger.info(`[SubscriptionDetectionService] Detected ${detectedSubscriptions.length} potential subscriptions`);
      
      return detectedSubscriptions;
    } catch (error) {
      logger.error("[SubscriptionDetectionService] Error detecting subscriptions:", error);
      if (error instanceof AppError) {
        throw error;
      }
      throw new AppError("Failed to detect subscriptions", 500);
    }
  }
   ...
   */
  private normalizeMerchantName(name: string | null | undefined): string {
    if (!name) return "";
   ...
   */
  private findKnownSubscriptionService(merchantName: string): { name: string; logoUrl?: string; typicalFrequency: "monthly" | "weekly" | "biweekly" | "semimonthly" | "daily" } | null {
   ...
   */
  private calculateFrequency(dates: Date[]): "monthly" | "weekly" | "biweekly" | "semimonthly" | "daily" {
   ...
   */
  private calculateBillingDay(frequency: "monthly" | "weekly" | "biweekly" | "semimonthly" | "daily", dates: Date[]): number | undefined {
   ...
   */
  private calculateDateRegularity(dates: Date[]): number {
