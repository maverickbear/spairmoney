...
import { planFeaturesSchema, type PlanFeatures } from "@/lib/validations/plan";
...
export function normalizeAndValidateFeatures(
  ...
  const validationResult = planFeaturesSchema.safeParse(parsedFeatures);
  
  if (!validationResult.success) {
    log.error("Plan features validation failed:", {
      planId,
